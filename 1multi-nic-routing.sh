...
# 在每次启动后强制刷新 IPv4 和 IPv6 路由规则
echo "🛠️ 刷新 NIC1 的 IPv4 和 IPv6 路由规则..."
# 处理 NIC1 的 IPv4 路由
NIC1_IPV4=$(ip -o -4 addr show dev ens4 | awk '{print $4}' | cut -d/ -f1)
if [ -n "$NIC1_IPV4" ]; then
  ip route replace default via "$GATEWAY" dev ens4
fi

# 处理 NIC1 的 IPv6 路由
NIC1_IPV6=$(ip -6 addr show dev ens4 | grep inet6 | awk '{print $2}' | head -n1)
if [ -n "$NIC1_IPV6" ]; then
  ip -6 route add default via "$LLGW" dev ens4
fi
