...
#
# 自动重启脚本：
# /root/.acme-auto/reboot-after-renew.sh
#
# ============================================================
...
REBOOT_SCRIPT="$CONF_DIR/reboot-after-renew.sh"
FIRST_ISSUE_MARKER="$CONF_DIR/.cert_installed_once"
...
MARKER="/root/.acme-auto/.cert_installed_once"

# ============================================================
# 第一次执行
#
# acme.sh 在首次 --install-cert 时也可能执行 reloadcmd。
# 第一次只创建标记，不重启。
# ============================================================
...
#
# 表示已经完成首次证书安装。
# 这里认为是后续证书更新/续签后的 reload。
# ============================================================

nohup sh -c 'sleep 10 && /sbin/reboot' \
    >/dev/null 2>&1 &
...
configure_new() {
    ...
    echo
    echo "Zone ID："
    echo "  适合明确指定某一个 Cloudflare Zone。"
    echo
    echo "Account ID："
    echo "  适合同一 Cloudflare 账户下的多个域名。"
    echo
    echo "如果不确定，直接回车即可。"
    echo

    read -r -p "Cloudflare Zone ID（可留空）: " CF_ZONE_ID
    CF_ZONE_ID="$(echo "$CF_ZONE_ID" | xargs)"

    read -r -p "Cloudflare Account ID（可留空）: " CF_ACCOUNT_ID
    ...
    ESC_CF_TOKEN="${CF_TOKEN//\'/\'\\\'\'}"
    ESC_CF_ZONE_ID="${CF_ZONE_ID//\'/\'\\\'\'}"
    ESC_CF_ACCOUNT_ID="${CF_ACCOUNT_ID//\'/\'\\\'\'}"
    ESC_DOMAIN="${DOMAIN//\'/\'\\\'\'}"
    ESC_DOMAINS="${DOMAINS//\'/\'\\\'\'}"
    ESC_ACME_EMAIL="${ACME_EMAIL//\'/\'\\\'\'}"

    cat > "$CONF_FILE" <<EOF
CF_TOKEN='$ESC_CF_TOKEN'
CF_ZONE_ID='$ESC_CF_ZONE_ID'
CF_ACCOUNT_ID='$ESC_CF_ACCOUNT_ID'
CERT_MODE='$CERT_MODE'
DOMAIN='$ESC_DOMAIN'
DOMAINS='$ESC_DOMAINS'
ACME_EMAIL='$ESC_ACME_EMAIL'
EOF
